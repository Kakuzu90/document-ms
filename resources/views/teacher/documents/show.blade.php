        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
                        ...
                        <div class="space-y-5">
                            ...
                            @empty
                                <div class="text-center py-10 bg-white border border-surface-200 rounded-xl shadow-sm">
                                    ...
                                    <p class="text-sm text-surface-500 font-medium">No comments have been left on this document yet.</p>
                                </div>
                            @endforelse
                        </div>
                            ...
                            @if($document->status->value !== 'reviewed')
                                <form method="POST" action="{{ route('teacher.comments.store', $document) }}">
                                    ...
                                    <div>
                                        ...
                                        <textarea id="body" name="body" rows="3" class="form-input mt-1 w-full bg-white border-surface-300 rounded-xl focus:border-primary-500 focus:ring-primary-500 shadow-sm resize-none transition-colors" required minlength="5" maxlength="2000" placeholder="Type your comment or reply here..."></textarea>
                ...
                <div class="lg:col-span-1 space-y-6 lg:sticky lg:top-8">
                    ...
                    <div class="card animate-fade-in relative z-30 shadow-md border-t-4 border-t-primary-500 overflow-hidden" style="animation-delay: 100ms;">
                        ...
                        <div class="p-6 bg-surface-50/50">
                            @if($document->status->value === 'submitted')
                                <a href="{{ route('teacher.documents.revise', $document) }}" class="inline-flex w-full items-center justify-center gap-2 px-4 py-2.5 bg-primary-600 border border-transparent rounded-lg font-semibold text-white hover:bg-primary-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500 transition-all shadow-sm">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12"></path></svg>
                                    Update File
                                </a>
                                <p class="text-xs text-surface-500 mt-3 text-center">You can update this file before it is reviewed.</p>
                            @elseif($document->status->value === 'needs_revision')
                                <a href="{{ route('teacher.documents.revise', $document) }}" class="inline-flex w-full items-center justify-center gap-2 px-4 py-2.5 bg-amber-600 border border-transparent rounded-lg font-semibold text-white hover:bg-amber-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-amber-500 transition-all shadow-sm">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12"></path></svg>
                                    Upload Revision
                                </a>
                                <p class="text-xs text-surface-500 mt-3 text-center">An admin has requested changes.</p>
                            @else
                                <div class="text-center py-2">
                                    <svg class="w-12 h-12 text-surface-300 mx-auto mb-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                                    <p class="text-sm font-medium text-surface-900">No actions available.</p>
                                    <p class="text-xs text-surface-500 mt-1">This document is currently <span class="font-semibold">{{ $document->status->label() }}</span>.</p>
                                </div>
                            @endif
                        </div>
                    </div>
                    ...
                    @if(isset($document->statusHistories) && $document->statusHistories->isNotEmpty())
